...
password_list = source.split("\n")


def valid_minmax(password_declaration):
...
def valid_count(passwords, pred):
    return sum(1 for p in passwords if pred(p))


test_valid = valid_count(password_list, valid_minmax)
print(f"Part 1 test case valid count is {test_valid}")
assert test_valid == 2

real_source = """5-9 g: ggccggmgn
...
real_password_list = real_source.split("\n")
real_valid = valid_count(real_password_list, valid_minmax)
...
print(f"Test case valid count is {test_valid}")
assert test_valid == 1
